...
class OrganizerDatatable
{
    ...
    public function data($request)
    {
        ...
        {
            $posts = Organizer::offset($start)->limit($limit)->orderBy($order, $dir)->get();
        }
        else
        {
            $search = $request->input('search.value');
            $posts  = Organizer::where('name', 'LIKE', "%{$search}%")
                 
                ->offset($start)->limit($limit)->orderBy($order, $dir)->get();

            $totalFiltered = Organizer::where('name', 'LIKE', "%{$search}%")
