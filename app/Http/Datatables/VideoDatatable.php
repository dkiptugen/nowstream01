        ...
        private function button($post, $request)
            {
                ...
                if ($request->user()->can('edit_video'))
                    {
                        $button .= '<a class="text text-dark" href="' . route('video.edit', ['video'=>$post->uuid]) . '" data-toggle="tooltip" title="Edit User">
                <i class="fas fa-edit"></i> Edit
                </a>';
                    }
                if ($request->user()->can('destroy_video'))
                    {
                        $button .= '<form id="delete-form-' . $post->id . '" action="' . route('video.destroy',['video'=>$post->uuid]) . '" method="POST" class=" create-form my-0 py-0">
