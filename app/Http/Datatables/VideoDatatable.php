        ...
        private function button($post, $request)
            {
                ...
                if ($request->user()->can('edit_video'))
                    {
                        $button .= '<a class="text text-dark" href="' . route('backend.video.edit', ['video' => $post->id]) . '" data-toggle="tooltip" title="Edit User">
                                    <i class="fas fa-edit"></i> Edit
                                    </a>';
                    }
                if ($request->user()->can('destroy_video'))
                    {
                        $button .= '<form id="delete-form-' . $post->id . '" action="' . route('backend.video.destroy', ['video' => $post->id]) . '" method="POST" class=" create-form my-0 py-0">
