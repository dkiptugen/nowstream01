            ...
            private function button($post, $request)
                {
	                ...
	                if ($request->user()->can('view_event_subscription'))
		                {
			                $button .= '<a class="text text-dark mr-2" href="' . route('event.subscription.index', $post->id) . '" data-toggle="tooltip" title="Show Subscriptions">
                                                <i class="fas fa-paperclip"></i>
                                                </a>';
		                }
	                if ($request->user()->can('view_event_transaction'))
		                {
			                $button .= '<a class="text text-dark mr-2" href="' . route('event.transaction.index',$post->id) . '" data-toggle="tooltip" title="Show Transactions">
                   ...
                   /* if ($request->user()->can('view_stream'))
                    ...
                    if ($request->user()->can('create_video'))
                        {
                            $button .= '<a class="text text-dark" href="' . route('event.video.create', $post->id) . '" data-toggle="tooltip" title="Upload Video">
