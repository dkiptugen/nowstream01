        ...
        private function button($post, $request)
            {
                ...
                if ($request->user()->can('edit_stream'))
                    {
                        $button .= '<a class="text text-dark" href="' . route('backend.stream.edit', ['stream'=> $post->uuid]) . '" data-toggle="tooltip" title="Edit User">
                <i class="bx bx-pencil"></i> Edit
                </a>';
                    }
                if ($request->user()->can('destroy_stream'))
                    {
                        $button .= '<form id="delete-form-' . $post->id . '" action="' . route('backend.stream.destroy', ['stream'=> $post->uuid]) . '" method="POST" class=" create-form my-0 py-0">
