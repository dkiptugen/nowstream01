...
Route::middleware(['detectCountry'])->group(function () {
	...
	Route::get('social/{social}', [AuthController::class, 'redirectToProvider'])->name('auth.social');
	Route::get('social/{social}/callback', [AuthController::class, 'handleProviderCallback'])->name('auth.social_callback');
	Route::any('social/{social}/delete', [AuthController::class, 'deleteProviderCallback'])->name('auth.social_delete');
	Route::post('/record-watch-history/{video}', [StreamVideoController::class, 'recordWatchHistory']);
	// Authentication routes
	Route::get('/partner/register', [AuthsController::class, 'partner']);
	Route::get('/user/register', [AuthsController::class, 'showRegisterForm']);
	Route::post('/user/register', [AuthsController::class, 'register'])->name('user.register');
	Route::get('/user/login', [AuthsController::class, 'showPhoneLoginForm'])->name('phonelogin.form');
	Route::post('/user/phone-login', [AuthsController::class, 'phoneLogin'])->name('phonelogin');
	Route::get('/user/phone-resend', [AuthsController::class, 'phoneResend'])->name('phoneresend');
	Route::post('/user/otp_verification', [AuthsController::class, 'otp_verify'])->name('otp_verification');
	Route::post('/user/login', [AuthsController::class, 'login'])->name('new.login');
	...
	Route::get('/email/verify/{id}/{hash}', [AuthsController::class, 'verifyEmail'])
