<div class="">
  <div class="card radius-5 comment sticky-top">
          ...
          <div class="card-body d-flex py-2 border-top px-2 mx-0 w-100 justify-content-between comment-item">
            <div class="d-flex">
              <div class="align-self-center text-center">
                <img src="{{ $comment_user->image }}" height="50" class="w-100 d-block w-100 aspect1" alt="...">
              </div>
              <div class="mx-1 mx-md-2">
                <div class="media-body">
                  <h6 class="my-0">{{ $comment_user->name }}</h6>
                  <p class="mb-0">
                    @php
    ...
    <div class="card-body row border-top px-0 mt-70 mx-0">
      @auth
        {{-- Pass the actual model you are commenting on --}}
        <form action="{{ route('comment.post', [
            'commentableType' => get_class($content),   // Example: Content model
            'commentableId'   => $content->uuid
        ]) }}" method="POST">
          @csrf
      ...
      @else
        <p class="text-center mt-3">
          Please <a href="{{ route('user.login') }}">login</a> to post a comment.
        </p>
      @endauth
    </div>
